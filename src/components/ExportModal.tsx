      ...
      <div className="studio-glass w-full max-w-2xl rounded-2xl border border-white/10 p-6 flex flex-col gap-5 shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 px-3 py-1.5 bg-studio-900 border border-white/10 rounded-lg text-xs font-mono text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-cyan-400"
            />
            <button
              type="button"
              onClick={handleApplyQuickRange}
              className="px-3 py-1.5 bg-studio-800 hover:bg-studio-700 text-cyan-400 border border-white/5 rounded-lg text-xs font-mono cursor-pointer"
            >
              Select Subset
            </button>
          </div>
          ...
          <div className="max-h-40 overflow-y-auto pr-1 flex flex-col gap-1.5 bg-studio-900/50 p-2.5 rounded-xl border border-white/5">
            {tracks.length === 0 ? (
                  ...
                  <div
                    ...
                    className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-all ${
                      isSelected
                        ...
                        : 'bg-studio-950/40 border border-white/5 text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-mono text-slate-400 flex-shrink-0">
                      ...
                      <span>•</span>
                      <span className={track.pitchCents !== 0 ? 'text-cyan-400' : ''}>
            ...
            <button
              ...
              className={`p-3 rounded-xl border text-left transition-all cursor-pointer ${
                useTrackSettings
                  ...
                  : 'bg-studio-900/40 border-white/5 text-slate-400 hover:text-slate-200'
              }`}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-white/5">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-mono text-slate-400">BITRATE QUALITY:</label>
            <select
              value={kbps}
              onChange={(e) => setKbps(Number(e.target.value))}
              className="bg-studio-900 border border-white/10 rounded-lg px-3 py-2 text-xs font-mono text-cyan-400 focus:outline-none focus:border-cyan-400"
            >
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-mono text-slate-400">OUTPUT FILENAME:</label>
            <input
              type="text"
              value={filename}
              onChange={(e) => setFilename(e.target.value)}
              className="bg-studio-900 border border-white/10 rounded-lg px-3 py-2 text-xs font-mono text-slate-200 focus:outline-none focus:border-cyan-400"
            />
          </div>
        </div>

        {/* Summary Info */}
        <div className="flex items-center justify-between text-xs font-mono text-slate-400 bg-studio-900/60 px-3 py-2 rounded-lg border border-white/5">
        ...
        {isExporting && (
          <div className="flex flex-col gap-2 p-3 rounded-xl bg-cyan-500/10 border border-cyan-500/30">
        ...
        {downloadUrl && !isExporting && (
          <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xs font-mono text-emerald-400 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4" />
              <span>Export Complete! Download initiated.</span>
            </div>
            <a
              href={downloadUrl}
              download={filename}
              className="px-3 py-1 bg-emerald-500 text-black font-bold rounded-lg hover:bg-emerald-400 transition-all"
            >
              Re-download
            </a>
          </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <button
            ...
            onClick={onClose}
            disabled={isExporting}
            className="px-4 py-2 text-xs font-mono text-slate-400 hover:text-white cursor-pointer disabled:opacity-30"
          >
            Cancel
          </button>
          <button
            ...
            className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-black font-semibold text-xs font-mono flex items-center gap-2 shadow-lg shadow-cyan-500/20 transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
          >
            <Download className="w-4 h-4" />
